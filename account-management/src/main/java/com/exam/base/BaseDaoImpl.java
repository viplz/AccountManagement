...
public class BaseDaoImpl implements BaseDao {
     ...
     * @return
     */
    public <T> Pagination<T> queryPagination(String sql, Object[] parameters, int begin, int pageSize, Class<T> entity) {
        // 将SQL语句进行分页处理
        String newSql = sQLPageHandle.handlerPagingSQL(sql, begin, pageSize);
        List<T> list = null;
        ...
        // 根据参数的个数进行差别查询
        Pagination<T> page = new Pagination<T>(begin, pageSize, totalList.isEmpty() ? 0 : totalList.size(), list);
